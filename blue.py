...
def detect_edges(frame):
    ...
    #cv2.imshow("HSV",hsv)
    lower_blue = np.array([90, 120, 0], dtype = "uint8")    #파란색의 하한값
    upper_blue = np.array([150, 255, 255], dtype="uint8")    #파란색의 상한값
    mask = cv2.inRange(hsv,lower_blue,upper_blue)    #파란색 이외의 색을 걸러내서 mask에 할당
